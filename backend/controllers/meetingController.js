...
exports.createMeeting = async (req, res, next) => {
  try {
    ...
    conversation.participants.forEach(participantId => {
      io.to(participantId.toString()).emit('meetingCreated', {
        meeting,
        conversationId
      });
    });
    ...
    res.status(201).json({
      success: true,
      data: meeting,
      meetingLink: `${process.env.CLIENT_URL}/meet/${meeting.meetingId}`
    });
  } catch (error) {
    next(error);
  }
...
exports.endMeeting = async (req, res, next) => {
  try {
    ...
    const isGroupAdmin = meeting.conversation.isGroupChat && 
...
exports.getMeetingHistory = async (req, res, next) => {
  try {
    ...
    const total = await Meeting.countDocuments({ conversation: conversationId });
